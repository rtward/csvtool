...
var rootCmd = &cobra.Command{
	...
	PersistentPreRun: func(cmd *cobra.Command, args []string) {
		...
		if err != nil {
			log.Fatal().Err(err).Msg("error reading verbose count flag")
		}
		...
		if err != nil {
			log.Fatal().Err(err).Msg("error reading header command flag")
		}
		...
		if hasHeader {
			...
			if err != nil {
				log.Fatal().Err(err).Msg("error reading header row")
			}
	...
	PersistentPostRun: func(cmd *cobra.Command, args []string) {
		...
		err := csvWriter.Error()
		if err != nil {
			log.Fatal().Err(err).Msg("error flushing CSV output")
		}
